          ...
          if(xhr.status == 200){
            ...
            if(!result.error){
              ...
              if(result.source == 'facebook'){
                if($('img').length > 0){
                  $('img').replaceWith('<video></video>')
                  $('video').attr('src', result.video_streams[0].thumb_url)
                  $('video').addClass("w-100")
                  $('video').addClass("border-radius-lg")
              ...
              if ('video_streams' in result && result.video_streams.length > 0){
                ...
                for(var i = 0; i < videos.length; i++){
                  $('#tbody_video').append('<tr>' +
                     ...
                     '</td>' +
                     '<td class="align-middle">' +
                        '<a class="btn mb-0" ' + (result.source == 'vk' ? 'href=' + videos[i].download_url : 'href="/download?token=' + videos[i].token + '"') + ' title="Download">' +
