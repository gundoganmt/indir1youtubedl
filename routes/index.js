...
async function facebook(meta){
  ...
  const video = new Video({
    web_url: meta.webpage_url,
    thumbnail: 'images/facebook_thumb.png',
    title: meta.title,
    dw_date: moment().format("MMM DD"),
    source: 'Facebook'
  })
...
router.get('/', async (req, res) => {
  const dw_list = ['youtube', 'twitter', 'facebook',  'instagram', 'reddit', 'vk', 'vlive', 'vimeo', 'soundcloud', 'izlesene']
...
router.post('/extractor', body.none(), async (req, res) =>{
  ...
  if(meta.extractor == 'youtube'){
    ...
    res.send(await facebook(meta.entries[0])) 
  }
  ...
  else if(meta.extractor == 'vk'){
    res.send(await vk(meta))
  }
});
